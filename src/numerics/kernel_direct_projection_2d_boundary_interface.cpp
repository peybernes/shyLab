...
void ProjectNodalIntensiveVariableOrder2BoundaryDirect(std::string BoundaryConditions,
  ...
  if (BoundaryConditions == "periodic") {
    ...
    if (variable_projected == "project_ux") {
      ProjectNodalIntensiveVariableOrder2UxWallBoundaryDirect(nx, ny, halo_width, dx, dy, dt, lag_cell_mass, out_cell_mass, u_velocity_pred, v_velocity_pred, in_variable, gradient_variable_x, gradient_variable_y, mass_flux_x, mass_flux_y, out_variable);
    }
    else if (variable_projected == "project_uy") {
      ProjectNodalIntensiveVariableOrder2UyWallBoundaryDirect(nx, ny, halo_width, dx, dy, dt, lag_cell_mass, out_cell_mass, u_velocity_pred, v_velocity_pred, in_variable, gradient_variable_x, gradient_variable_y, mass_flux_x, mass_flux_y, out_variable);
    }
  }
  else{
    ProjectNodalIntensiveVariableOrder2PeriodicBoundaryDirect(nx, ny, halo_width, dx, dy, dt, lag_cell_mass, out_cell_mass, u_velocity_pred, v_velocity_pred, in_variable, gradient_variable_x, gradient_variable_y, mass_flux_x, mass_flux_y, out_variable); 
