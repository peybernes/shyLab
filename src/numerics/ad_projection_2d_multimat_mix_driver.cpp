...
void AdProjection2dMultimaterialMixXDriver(const std::string BoundaryConditions,
					   const int nx,
					   const int ny,
  ...
  // ========================
  //     Projection X _ concentration c_1. (projection of mass*c_1 then back to c_1 )
  // ========================
  ...
  clock_gettime(CLOCK_REALTIME, &time_begin);
  MassProjectIntensiveVariableX(nx, ny , in_cell_mass, in_c_1, concentration_flux_x, out_cell_mass,
				out_c_1);
  clock_gettime(CLOCK_REALTIME, &time_end);
  time_project_energy_X.push_back(diff(time_begin, time_end));
...
void AdProjection2dMultimaterialMixYDriver(const std::string BoundaryConditions,
					   const int nx,
					   const int ny,
					   const int nb_faces_x,
					   const int nb_faces_y,
  ...
  clock_gettime(CLOCK_REALTIME, &time_begin);
  ReconstructIntensiveVariableFluxOrder2Y(nx, ny, halo_width, dx, dy, volume_fluxes_y, mass_flux_y, in_c_1, concentration_gradient,
					  concentration_flux_y);
  clock_gettime(CLOCK_REALTIME, &time_end);
  time_reconstruct_energy_o2_Y.push_back(diff(time_begin, time_end));
     
  clock_gettime(CLOCK_REALTIME, &time_begin);
  ReconstructIntensiveVariableFluxOrder2YBoundary(BoundaryConditions, nx, ny, halo_width, dx, dy, volume_fluxes_y, mass_flux_y, in_c_1, concentration_gradient,
						  concentration_flux_y);
  clock_gettime(CLOCK_REALTIME, &time_end);
  time_periodic_boundary.push_back(diff(time_begin, time_end));
  ...
  clock_gettime(CLOCK_REALTIME, &time_begin);
  MassProjectIntensiveVariableY(nx, ny , in_cell_mass, in_c_1, concentration_flux_y, out_cell_mass,
				out_c_1);
  clock_gettime(CLOCK_REALTIME, &time_end);
  time_project_energy_Y.push_back(diff(time_begin, time_end));
